# codeTrackerJS — Project Plan v2.4.3
...
> **Note:** In this document, “APIs” and “exports” refer to the *internal contracts* between the
> modules inside the `codeTrackerJS` package. They are not intended as a public, external API; they
> simply define how the orchestrator and helper modules call each other.
...
#### Required named exports (module contract)
...
- Ensure required schema and tables exist as defined below.
- Optionally apply a schema/table prefix, e.g., `test_`.

#### Database schema (canonical, required)

`schemaInit.mjs` MUST create (or verify) the following tables. Table names MUST honour `schemaPrefix` by prepending it (e.g. `test_modules`, `test_snapshots`, `test_functions`).

All columns, primary keys, and constraints are part of the contract and must not be changed without updating this project plan and applying a DB migration. **Self-tests for any module that touches the DB MUST treat this schema as authoritative and MUST fail fast if the live database does not match it.**
...
CREATE TABLE IF NOT EXISTS {prefix}snapshots (
  id          SERIAL PRIMARY KEY,
  module_id   INTEGER NOT NULL REFERENCES {prefix}modules(id) ON DELETE CASCADE,
  branch_name TEXT    NOT NULL,
  release_tag TEXT    NOT NULL,
  created_at  TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  UNIQUE (module_id, branch_name, release_tag)
);

-- <prefix>functions
CREATE TABLE IF NOT EXISTS {prefix}functions (
  id           SERIAL PRIMARY KEY,
  snapshot_id  INTEGER NOT NULL REFERENCES {prefix}snapshots(id) ON DELETE CASCADE,
...
Where `{prefix}` is the value of `schemaPrefix` passed to `initSchema` (empty string for production).

`storeSnapshot.mjs` relies on:

- `modules.id` and `snapshots.id` being the primary keys;
- `snapshots.module_id` referencing `modules.id`;
- `functions.snapshot_id` referencing `snapshots.id`;
- the `(module_id, branch_name, release_tag)` uniqueness constraint on `snapshots`.

`schemaInit` must create exactly this structure (or a superset that remains compatible with these contracts).


#### Required named exports (module contract)

> **Important:** `initSchema` is a *pure schema initializer*. It must be idempotent and
> must **not** run behavioural tests that intentionally trigger constraint violations
> (for example, inserting duplicate rows to test a UNIQUE index). Those tests belong
> only in the module’s self‑test block.
...
- If `CODE_TRACKER_TEST_ENABLE_DB === "false"`, log a short message and exit `0`.

- Otherwise:

  1. Obtain client via `getPgClient({ databaseOverride: CODE_TRACKER_TEST_DATABASE })` if set, otherwise default.
  2. Read `CODE_TRACKER_TEST_SCHEMA_PREFIX` into a local `schemaPrefix` (may be empty string).
  3. Call `await initSchema(client, { schemaPrefix })`. This call must be safe and idempotent and must **not** internally call any helper that deliberately inserts invalid or duplicate rows in order to test constraints.
  4. Query `information_schema.columns` (or `pg_catalog`) to confirm that **each required table and column** from the canonical schema section exists, including:
     - `<prefix>modules.slug`
     - `<prefix>snapshots.module_id`, `branch_name`, `release_tag`, `created_at`
     - `<prefix>functions.snapshot_id`, `file_path`, `export_name`, `params_json`
  5. If `schemaPrefix` is non-empty (e.g., `test_`), behaviourally verify the composite uniqueness constraint on `<prefix>snapshots` **from the self‑test block only**, using a helper such as `verifyUniqueConstraint(client, schemaPrefix)` that is **never called from `initSchema` itself**:
     - Clean up any existing test rows for the chosen slug/branch/tag before starting.
     - Insert a temporary module row, then insert one snapshot row for a fixed `(module_id, branch_name, release_tag)` triple. This must succeed.
     - Attempt to insert a second snapshot row with the **same** `(module_id, branch_name, release_tag)` triple. This must fail with Postgres error code `23505` (`unique_violation`). If it does not, throw an error such as:
       ```text
       "Expected UNIQUE (module_id, branch_name, release_tag) on <prefix>snapshots to reject duplicate rows"
       ```
     - Always clean up the temporary test rows at the end so repeated self‑tests and other modules (like `storeSnapshot`) can call `initSchema` without tripping over earlier data.
  6. If `schemaPrefix` is empty (production schema), the self-test MUST NOT drop or truncate tables and MUST NOT insert duplicate rows purely to test uniqueness. It may perform only safe existence checks as in step 4.
  7. In all cases, close the client before exiting. On any failure, log a clear error and exit non-zero rather than throwing uncaught exceptions.
  2. Read `CODE_TRACKER_TEST_SCHEMA_PREFIX` into a local `schemaPrefix` (may be empty string).
  3. Call `await initSchema(client, { schemaPrefix })`.
  4. Query `information_schema.columns` (or `pg_catalog`) to confirm that **each required table and column** from the canonical schema section exists, including:
     - `<prefix>modules.slug`
     - `<prefix>snapshots.module_id`, `branch_name`, `release_tag`, `created_at`
     - `<prefix>functions.snapshot_id`, `file_path`, `export_name`, `params_json`
  5. If `schemaPrefix` is non-empty (e.g., `test_`), behaviourally verify the composite uniqueness constraint on `<prefix>snapshots`:
     - Insert a temporary module row:
       ```js
       const { rows: [mod] } = await client.query(
         `INSERT INTO ${schemaPrefix}modules (slug) VALUES ($1) RETURNING id`,
         ["schemainit-selftest"]
       );
       const moduleId = mod.id;
       ```
     - Insert one snapshot row for a fixed `(module_id, branch_name, release_tag)` triple. This must succeed:
       ```js
       await client.query(
         `INSERT INTO ${schemaPrefix}snapshots (module_id, branch_name, release_tag)
          VALUES ($1, $2, $3)`,
         [moduleId, "selftest-branch", "v0.0.0"]
       );
       ```
     - Attempt to insert a second snapshot row with the **same** `(module_id, branch_name, release_tag)` triple. This must fail with Postgres error code `23505` (`unique_violation`). If it does not, throw an error such as:
       ```text
       "Expected UNIQUE (module_id, branch_name, release_tag) on <prefix>snapshots to reject duplicate rows"
       ```
     - The self-test MAY clean up the temporary rows at the end, but it is not required.
  6. If `schemaPrefix` is empty (production schema), the self-test MUST NOT drop or truncate tables and MUST NOT insert duplicate rows purely to test uniqueness. It may perform only safe existence checks as in step 4.
  7. In all cases, close the client before exiting. On any failure, log a clear error and exit non-zero rather than throwing uncaught exceptions.
---
...
- The README must mention that this package is implemented against **project plan v2.4.0**.
...
Expose a **read-only HTTP API** that serves the function metadata and snapshots already written to Postgres by `codeTrackerJS`. The API does **not** mutate data; it reuses the DB connection logic from `pgClient.mjs` and respects any `schemaPrefix` used at insert time.

### 9.2 Configuration

- Uses the same `CODE_TRACKER_PG_*` env vars as runtime modules.
- Additional vars:

  ```env
  CODE_TRACKER_API_HOST=0.0.0.0   # optional, defaults to 127.0.0.1
  CODE_TRACKER_API_PORT=3000      # required port to listen on
  CODE_TRACKER_API_SCHEMA_PREFIX= # optional, matches writer prefix (e.g. test_)
  ```

### 9.3 Behaviour + Endpoints

- **Read-only**: queries Postgres; never inserts/updates/deletes.
- **Slug handling**: `moduleSlug` must match the value derived by the orchestrator when data was stored.
- **Error handling**: `404` when module/snapshot not found; `400` for malformed params; `5xx` for DB failures with redacted messages.

Endpoints (JSON responses; timestamps in ISO 8601):

1. `GET /api/modules`
   - Returns all modules with latest snapshot info per module.
   - Example response:

     ```json
     [
       {
         "moduleSlug": "newsletterjs",
         "latestBranch": "main",
         "latestReleaseTag": "v1.2.0",
         "latestSnapshotCreatedAt": "2024-02-20T18:30:00.000Z"
       }
     ]
     ```

2. `GET /api/modules/:moduleSlug/snapshots`
   - Optional query `branchName` filters to a branch.
   - Returns all snapshots (branch + releaseTag + created_at) for the module.

3. `GET /api/modules/:moduleSlug/snapshots/:branchName/:releaseTag`
   - Returns snapshot summary derived from stored function rows:

     ```json
     {
       "moduleSlug": "newsletterjs",
       "branchName": "feature/email-metrics",
       "releaseTag": "v1.3.0-beta",
       "functionsCount": 12,
       "parametersCount": 21,
       "returnValuesCount": 12,
       "createdAt": "2024-03-01T10:00:00.000Z"
     }
     ```

4. `GET /api/modules/:moduleSlug/snapshots/:branchName/:releaseTag/functions`
   - Returns the stored `FunctionMetadata[]` for that snapshot using DB columns:

     ```json
     [
       {
         "filePath": "src/email/send.mjs",
         "exportName": "sendEmail",
         "params": [{ "name": "to", "type": "string" }],
...
- Reuse `getPgClient` and pass through `CODE_TRACKER_API_SCHEMA_PREFIX` to all queries.
- Keep one pooled connection or shared client per process; close gracefully on SIGINT/SIGTERM.
- Add lightweight request logging and input validation; avoid exposing SQL errors directly.
- Pagination is not required initially but responses should be structured so page params can be added without breaking clients (e.g., wrap arrays in objects later).
