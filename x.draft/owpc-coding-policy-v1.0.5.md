...
- **Database Schema (if the package uses a database)**:
  ...
  - Project plans MUST specify how self-tests verify the database schema:
    ...
    - For any schema that uses a dedicated test prefix (e.g., `test_`), self-tests are allowed to drop and recreate the prefixed tables if the live schema does not match the canonical schema.
    - For non-prefixed (production) schemas, self-tests MUST NOT modify existing tables; instead, they must fail with a clear error when a schema mismatch is detected.
